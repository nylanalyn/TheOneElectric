...
class RandomResponsePlugin:
    ...
    def _should_respond(self, trigger: dict, addressed_to_bot: bool, message: str) -> bool:
        if addressed_to_bot:
            return random.random() < self.targeted_response_chance
    ...
    async def handle_message(self, bot, nick: str, channel: str, message: str) -> bool:
        ...
        for trigger in self.triggers:
            if trigger["pattern"].search(message):
                if self._should_respond(trigger, addressed_to_bot, message) and self._cooldowns_allow(nick, channel, now):
